<template>
  <el-container>
    <div v-if="opt=='graphs'" style="width:100%;height:100px;">
        ...
        <el-row :gutter="35">
            <div @click="addDialogVisible = true" style="float:left;margin-left:20px;margin-top:10px;">
                <el-card shadow="hover" style="width: 200px;height: 200px;margin-bottom:10px;"> 
                    <i style="font-size:20px;float:left;margin-top:50px;" class='el-icon-plus'>   添加新图谱</i>
                </el-card>
            </div>
            ...
            <div @click="graphDetail(graph.graph_id,graph.graph_name,graph.domain_id)">
                <el-card shadow="hover" style="width: 200px;height: 200px;margin-bottom:10px;margin-top:10px;">
                <el-image style="width: 100px; height: 100px; margin-left:20%;margin-top:5%;"
                src="https://ss0.bdstatic.com/70cFuHSh_Q1YnxGkpoWK1HF6hhy/it/u=3243406373,3135595743&fm=26&gp=0.jpg"></el-image>
    ...
    <div v-if="opt=='graphDetail'" style="width:100%;height:100px;">
        ...
        <div style="width:20%;float:right;">
            ...
            <el-button slot="append" icon="el-icon-search" @click="searchVertex()"></el-button>
            </el-input>
        </div>
        </el-card>
        <div style="margin-top:5px;">
            <el-col :span="3">
                <el-button @click="addVertexVisible=true" size="mini" type="warning" icon="el-icon-star-off" style="margin-left:10px;margin-right:10px;margin-top:10px;margin-bottom:10px;" circle></el-button>添加实体类型
                <el-menu default-active="2" class="el-menu-vertical-demo" style="margin-top:3px;">
                    <el-menu-item>实体类型列表</el-menu-item>
                    <el-menu-item v-for="collection in collections_list" :key="collection" :unique-opened="true" @click="showVertexs(collection,1)" >
                    <template slot="title">
        ...
        <el-card shadow="never">
            <!-- 显示某类型下的所有实体 -->
            <div v-if="search==false" style="height:100%">
                <el-button @click="addVertexVisible=true" size="mini" type="primary" icon="el-icon-plus" style="margin-left:10px;margin-right:10px;margin-bottom:10px;" circle>
                </el-button>添加{{collection}}节点
                <el-table :data="vertexs_list" border style="width: 100%" size="mini">
                    <el-table-column type="index" width="50"></el-table-column>
                    <el-table-column prop="_id" label="ID" width="310"></el-table-column>
                    <el-table-column prop="name" label="节点名称" width="310"></el-table-column>
                    <el-table-column label="操作" width="400">
                        <template slot-scope="scope">
                            <el-button type="success" icon="el-icon-picture" circle @click="graphVisible=true;vertex=scope.row"></el-button>
                            <el-button type="primary" icon="el-icon-edit" circle @click="editVisible=true;vertex=scope.row"></el-button>
                            <el-popconfirm
                            ...
                            >
                            <el-button  style="margin-left:10px" type="danger" slot="reference" icon="el-icon-delete" circle @click="vertex=scope.row"></el-button>
                            </el-popconfirm>
                        </template>
                    </el-table-column>
                </el-table>
            ...
            <!-- 显示用户搜索的实体节点 -->
            <div v-else>
                <div style="margin-bottom:10px;"><b>查询到:{{searchLen}}个相关节点</b></div>
                ...
                <el-table :data="searchResults" border style="width: 100%" height="500" size="mini">
                    <el-table-column type="index" width="50"></el-table-column>
                    <el-table-column prop="collection" label="节点类型" width="200"></el-table-column>
                    <el-table-column prop="_id" label="ID" width="250"></el-table-column>
                    <el-table-column prop="name" label="节点名称" width="250"></el-table-column>
            ...
            <!-- 以中心节点方式展示图谱 -->
            <el-dialog title="图谱显示" :fullscreen=fullscreen :visible.sync="graphVisible" @opened="openDialog(maxDepth)" @close="closeFullScreen()">
                ...
                <!-- 图中显示层数 -->
                    ...
                    <div style="position:absolute;top: 16px;right:60px;float:left"><el-link @click="fullGraph()" :underline=false><i class="el-icon-full-screen"></i></el-link></div>
            </el-dialog>
            <!-- 展示该图谱的中心实体对应的关系列表（编辑功能） -->
            <el-dialog :title="'图谱编辑-中心节点：'+ vertex._id" :visible.sync="editVisible" @opened="openDialog('1')">
                <el-table :data="graph_links_sorted" style="width: 100%">
                    ...
                    <template slot-scope="scope">
                        <span style="margin-left: 10px">{{ scope.row.relation }}        
                            <el-button slot="reference" size="mini" type="info" icon="el-icon-plus" style="margin-left:10px" circle @click="addNewEdgeVisible=true"></el-button>
                        </span>
                        <!-- 新增与中心实体有关的节点 -->
                        <el-dialog title="新增关系节点" :visible.sync="addNewEdgeVisible">
                            <el-form :model="newEdge">
                                ...
                                <el-form-item label="选择节点" prop="resource" label-width="100px">
                                    <el-autocomplete v-model="newEdge.name" :fetch-suggestions="querySearchAsync" placeholder="请输入内容" @select="handleSelectNewEdge"></el-autocomplete>
                                </el-form-item>
                                ...
                                <div v-if="node.from_or_to=='source'">
                                    <div slot="reference" class="name-wrapper" style="margin-bottom:3px;">
                                        <el-tag size="medium">{{ node.node_name }} 
                                            <i class="el-icon-close" @click="node.node_deleteVisible=true"></i>
                                        </el-tag>
                                    </div>
                ...
                <el-dialog title="新增关系类型" :visible.sync="addRelationVisible">
                    <el-form :model="newRelation">
                        ...
                        <el-form-item label="选择节点" prop="resource" label-width="100px">
                            <el-autocomplete v-model="newRelation.name" :fetch-suggestions="querySearchAsync" placeholder="请输入内容" @select="handleSelectNewRelation"></el-autocomplete>
                        </el-form-item>
                ...
                <!-- 重命名某中心实体 -->
                <div><el-button type="text" @click="renameVertexVisible = true">重命名该中心节点名称</el-button></div>
        ...
        async showEditGraph(maxDepth){
            ...
            if(this.graphVisible){
                ...
                for(var i=0;i<this.graph_links.length;i++){
                    ...
                    console.log(this.graph_links[i])
